...
const publicBaseUrl = (process.env.PUBLIC_BASE_URL || 'http://192.168.3.20:1222').replace(/\/$/, '')

function filePathToPublicUrl(filePath) {
  return publicBaseUrl + encodeURI(filePath)
}
	...
	if (storage.indexes.itemId.has(itemIdLocal)) {
		...
		for (var entry of entries.values()) {
			...
			if (args.type === f.type && args.id === getFileVideoId(f)) streams.push({
				id: filePathToPublicUrl(f.path),
				url: filePathToPublicUrl(f.path),
